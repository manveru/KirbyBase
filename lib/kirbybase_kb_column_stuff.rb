...
#                    @#{field_name} = nil
    ...
    #-----------------------------------------------------------------------
    #++
    # Create column instances for table.
    #
    def create_columns
        @columns = []

        @col_names.zip(@col_types, @col_indexes, @col_defaults,
         @col_requireds, @col_extras
         ).each do |name, index, data_type, default, required, extra|
            if extra.has_key?('Lookup')
                col = KBLookupColumn(name, data_type)
                col.lookup_table, col.lookup_column = extra['Lookup'].split(
                 '.')
                col.index = index
            elsif extra.has_key?('Link_many')
                col = KBLinkManyColumn(name, data_type)
                col.key_column, rest = extra['Link_many'].split('=')
                col.many_table, col.fk_column = rest.split('.')
            elsif extra.has_key?('Calculated')
                col = KBCalculatedColumn(name, data_type)
                col.calculation = extra['Calculated']
            else
                col = KBColumn(name, data_type)
                col.default = default
...
#---------------------------------------------------------------------------
class KBColumn
    attr_accessor :name, :data_type, :required, :default
...
# KBLookupColumn
#---------------------------------------------------------------------------
class KBLookupColumn < KBColumn
    attr_accessor :lookup_table, :lookup_column
...
#    def ^(other)
#        return false if other.nil? or not other
#        return true
#    end
